<div class="mb-8">
    ...
    <p class="text-gray-400">
        Update SaaS subscription plan information.
    </p>
...
<div class="bg-gray-900 border border-gray-800 rounded-2xl p-6 max-w-5xl">
    ...
    <form action="/mealbox/public/?url=admin-subscription-plan-update&id=<?= $plan[
        'id'
    ] ?>"
          method="POST"
          class="space-y-6">
                ...
                <?php foreach ($meals as $meal): ?>
                    ...
                    <label class="bg-black border border-gray-700 rounded-xl p-4 flex items-start gap-3 cursor-pointer hover:border-yellow-500 transition">
                        ...
                        <input type="checkbox"
                               name="meal_ids[]"
                               value="<?= $meal['id'] ?>"
                               <?= in_array($meal['id'], $selectedMeals)
                                   ? 'checked'
                                   : '' ?>
                               class="mt-1">
